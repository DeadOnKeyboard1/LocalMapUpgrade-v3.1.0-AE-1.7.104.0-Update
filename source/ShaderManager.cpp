...
#include "ISLocalMap.hlsl.h"
	...
	ID3D11PixelShader* ShaderManager::CompilePixelShader(const char* a_pixelShaderSrc,
																   const std::vector<const char*>& a_defineNames)
	{
		...
		ID3D11Device* device = RE::BSGraphics::Renderer::GetSingleton()->data.forwarder;

		ID3D11PixelShader* pixelShaderProgram = nullptr;
	...
	{
		isFogOfWarEnabled = settings::mapmenu::localMapFogOfWar;
		style = settings::mapmenu::localMapColor ? PixelShaderProperty::Style::kColor : PixelShaderProperty::Style::kBlackNWhite;
		SetPixelShaderProperties(shape, style);
	}
	...
	void ShaderManager::SetPixelShaderProperties(PixelShaderProperty::Shape a_shape, PixelShaderProperty::Style a_style)
	{
		...
		auto shader = isFogOfWarEnabled ? shaders.fogOfWar : shaders.noFogOfWar;
		if (localMapPixelShader && shader) {
			localMapPixelShader->shader = reinterpret_cast<RE::ID3D11PixelShader*>(shader);
		}
		if (singleton) {
