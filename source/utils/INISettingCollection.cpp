...
namespace utils
{
	...
	bool INISettingCollection::ReadFromFile(std::string_view a_fileName)
	{
		...
		return ReadFromPath(std::filesystem::path(executable.data()).parent_path() / "Data/SKSE/Plugins" / a_fileName);
	}

	bool INISettingCollection::ReadFromPath(const std::filesystem::path& iniPath)
	{
