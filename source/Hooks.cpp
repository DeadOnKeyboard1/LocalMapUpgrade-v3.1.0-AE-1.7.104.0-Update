...
bool ProcessButton(RE::LocalMapMenu::InputHandler* a_localMapInputHandler, RE::ButtonEvent* a_event)
{
	...
	struct RestoreEventName {
		...
		~RestoreEventName() { event->userEvent = name; }
	} restoreEventName{ buttonEvent, buttonEvent->userEvent };
	...
	const bool readyBefore = localMapMenu.showingMap && localMapMenu.controlsReady && !isLocationFinderShown;
	...
	if (localMapMenu.showingMap && localMapMenu.controlsReady && !isLocationFinderShown)
	{
		if (buttonEvent)
		{
				...
				) || buttonEvent->userEvent == userEvents->placePlayerMarker)
			{
				...
				auto menu = a_localMapInputHandler->localMapMenu;
				const bool inBounds = device != RE::INPUT_DEVICE::kMouse ||
					(cursor->cursorPosX >= menu->topLeft.x && cursor->cursorPosX <= menu->bottomRight.x &&
					 cursor->cursorPosY >= menu->topLeft.y && cursor->cursorPosY <= menu->bottomRight.y);
				if (LMU::ShouldPlaceMarker(readyBefore, localMapMenu.showingMap && localMapMenu.controlsReady,
					true, inBounds, buttonEvent->Value(), buttonEvent->HeldDuration()))
				{ 
					float wndPointX{};
					float wndPointY{};
