...
public:
	RE::BSEventNotifyControl ProcessEvent(const RE::MenuOpenCloseEvent* event, RE::BSTEventSource<RE::MenuOpenCloseEvent>*) override
	{
		if (event && ((!event->opening && event->menuName == RE::JournalMenu::MENU_NAME) ||
			(event->opening && event->menuName == RE::MapMenu::MENU_NAME))) {
			SKSE::GetTaskInterface()->AddTask([] { settings::Reload(); });
		}
...
void SKSEMessageListener(SKSE::MessagingInterface::Message* a_msg)
{
	...
	else if (a_msg->type == SKSE::MessagingInterface::kDataLoaded) 
	{
		...
		RE::UI::GetSingleton()->AddEventSink<RE::MenuOpenCloseEvent>(&settingsObserver);
	}
