...
namespace RE
{
	...
	bool NiCamera__WindowPointToRay(NiCamera* a_camera, std::int32_t a_x, std::int32_t a_y, NiPoint3& a_origin, NiPoint3& a_dir, float a_windowWidth, float a_windowHeight)
	{
									 ...
									 const BSTArray<BSString>& a_options)
	{
		...
		static REL::Relocation<func_t> func{ REL::VariantID(51421, REL::Module::get().version() < REL::Version(1, 6, 1130, 0) ? 52270 : 442726, 0x8D82D0) };
...
namespace LMU
{
	...
	void PlaceMarkerImmediate(RE::LocalMapMenu* a_localMapMenu, float a_wndPointX, float a_wndPointY)
	{
		...
		RE::NiPoint3 rayDir;
		if (RE::NiCamera__WindowPointToRay(localMapCamera, static_cast<std::int32_t>(wndPointX), static_cast<std::int32_t>(wndPointY), rayOrigin, rayDir, localMapViewWidth, localMapViewHeight))
		{
			...
			if (GetRayCollisionPosition(rayOrigin, rayDir, markerPos))
			{
				...
				RE::ObjectRefHandle playerMapMarker = GetPlayerInfo(player).playerMapMarker;
				RE::PlayerCharacter::TeleportPath* playerMarkerTeleportPath = GetPlayerInfo(player).playerMarkerPath;
				...
				if (playerMapMarker && playerMarkerTeleportPath)
				{
					...
					if (auto mapMenu = RE::UI::GetSingleton()->GetMenu<RE::MapMenu>())
					{
						auto& mapMenuMarkers = *reinterpret_cast<RE::BSTArray<RE::MapMenuMarker>*>(&mapMenu->GetRuntimeData2().unk30470);
