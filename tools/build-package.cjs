...
const vmad = cat(u16(5), u16(2), u16(1), script,
  u8(2), u16(0), str(''), // extra bind data: no quest stage fragments
  u16(1), u16(0), u16(0), u32(questID), // one alias, index 0 on this quest
...
for (const relative of ['SKSE/Plugins/LocalMapUpgrade.ini',
  'Interface/InfinityUI/Map/WorldMap/LocalMapMenu/IconDisplayExtension.swf',
  'Interface/InfinityUI/Map/WorldMap/LocalMapMenu/!assets/IconDisplayExtensionArt.swf']) {
  write(relative, fs.readFileSync(path.join(root, '..', relative)));
}
write('SKSE/Plugins/LocalMapUpgrade.dll', fs.readFileSync(path.join(root, 'build/Release/LocalMapUpgrade.dll')));
write('SKSE/Plugins/LocalMapUpgrade.pdb', fs.readFileSync(path.join(root, 'build/Release/LocalMapUpgrade.pdb')));
write('SKSE/Plugins/LocalMapUpgrade-SimpleIni-LICENSE.txt', fs.readFileSync(path.join(root, 'include/third_party/SimpleIni-LICENSE.txt')));
write('MCM/Config/LocalMapUpgrade/settings.ini', fs.readFileSync(path.join(root, '../SKSE/Plugins/LocalMapUpgrade.ini')));
...
let cursor = 6;
function readString() { const n = v.readUInt16LE(cursor); cursor += 2; const s = v.toString('utf8', cursor, cursor + n); cursor += n; assert.ok(!s.includes('\0')); return s; }
...
console.log('PASS: 11 settings covered; defaults match; ESL quest/VMAD/player alias/PEX/SEQ validated.');
